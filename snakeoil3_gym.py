        ...
        while True:
            ...
            try:
                ...
                if n_fail < 0:
                    ...
                    #cmd2=" \"nohup bash -c 'pkill python'\""
                    #os.system("sshpass -p \"qwerty1234\" ssh -o StrictHostKeyChecking=no meha@10.2.36.169"+cmd+cmd2)
                    time.sleep(1.0)
...
def intelligent_driver(target_velocity, max_acceleration):
        return
...
def drive_example(c, target_speed):
    ...
    print(min(opponents))
    if opponents[0] < min_dist:
        R[u'accel'] = 0.
        R[u'brake'] = 0.1

    if opponents[18] < min_dist:
        R[u'accel'] = 0.3

    if opponents[0] < min_dist and opponents[18] < min_dist:
        R[u'accel'] = 0.
        R[u'brake'] = 0.

    # Lane change 

    # with very low probability change lane 
...
def create_clients(num_clients=6):
    port = 3101
    clients = []
    for i in num_clients:
        clients.append(Client(port+i))
    return clients
...
if __name__ == u"__main__":
    ...
    C9 = Client(p=3109)
    C10 = Client(p=3110)
    for step in range(C1.maxSteps,0,-1):
        C1.get_servers_input(step)
        drive_example(C1, 50)
        C1.respond_to_server()
        C2.get_servers_input(step)
        drive_example(C2, 50)
        C2.respond_to_server()
        C3.get_servers_input(step)
        drive_example(C3, 50)
        C3.respond_to_server()
        C4.get_servers_input(step)
        drive_example(C4, 50)
        C4.respond_to_server()
        C5.get_servers_input(step)
        drive_example(C5, 40)
        C5.respond_to_server()
        C6.get_servers_input(step)
        drive_example(C6, 40)
        C6.respond_to_server()
        C7.get_servers_input(step)
        drive_example(C7, 40)
        C7.respond_to_server()
        C8.get_servers_input(step)
        drive_example(C8, 40)
        C8.respond_to_server()
        C9.get_servers_input(step)
        drive_example(C9, 30)
        C9.respond_to_server()
        C10.get_servers_input(step)
        drive_example(C10, 30)
        C10.respond_to_server()
